...
export function pcmToWav(chunks: readonly PcmData[]): Blob {
  ...
  let peak = 0;
  for (const c of chunks) for (const s of c.samples) { const a = Math.abs(s); if (a > peak) peak = a; }
  const scale = 1 / Math.max(0.01, peak);

  const pcm16 = new Int16Array(totalSamples);
  let offset = 0;
  for (const c of chunks) for (const s of c.samples) {
