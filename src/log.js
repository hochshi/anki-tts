...
export function logLine(level, ...args) {
    ...
    if (level === "error") {
        ...
        const config = document.getElementById("ttsConfigContainer");
        if (config)
            config.style.display = "block";
    }
}
export function installGlobalLogging() {
    if (window.__ttsLoggingInstalled)
        return;
    window.__ttsLoggingInstalled = true;
